    ...
    <div className="p-10 flex flex-col gap-8 w-full max-w-7xl mx-auto h-full overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-text-muted font-medium">Manage your time with surgical precision.</p>
        </div>
        ...
        {dates.map((date) => {
          ...
          return (
            <button
              ...
              className={cn(
                "flex flex-col items-center min-w-[70px] p-3 rounded-2xl border transition-all duration-300",
                isSelected 
        ...
        <div className="flex flex-col gap-6">
           <div className="flex items-center justify-between">
             ...
             <p className="text-xs text-text-muted font-medium uppercase tracking-wider">
               {isPast ? "Archive View" : isTooFarFuture ? "Beyond Horizon" : "Active Schedule"}
             </p>
           </div>
           ...
           {canAddTask ? (
             <form onSubmit={handleAddTask} className="relative group">
                ...
                <input 
                  type="text"
                  placeholder="Add a precision task for today..."
                  value={newTaskTitle}
                  onChange={(e) => setNewTaskTitle(e.target.value)}
                  className="w-full pl-12 pr-4 py-4 bg-surface/30 border border-border/50 rounded-2xl text-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/50 transition-all placeholder:text-text-disabled"
                />
             </form>
           ) : (
             <div className="p-4 rounded-2xl bg-info/5 border border-info/20 flex items-center gap-3 text-info">
               <Info className="h-5 w-5 shrink-0" />
               <p className="text-sm font-medium">
                 {isPast ? "This date is in the past. Tasks are read-only." : "Task scheduling is limited to the next 30 days."}
               </p>
             </div>
           )}
                ...
                {dailyTasks.length > 0 ? (
                  dailyTasks.map((task) => (
                    <motion.div
                      ...
                      exit={{ opacity: 0, x: 20 }}
                      className={cn(
                        "group flex items-center justify-between p-4 rounded-2xl border transition-all duration-300 bg-surface/20",
