...
export function MyTasksView() {
  ...
  return (
    <div className="p-10 flex flex-col gap-8 w-full max-w-7xl mx-auto h-full overflow-hidden">
      {/* Header Section */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-text-muted font-medium">Manage your personal workload and track progress.</p>
        </div>
        ...
        <div className="flex flex-col items-end gap-3">
          <div className="relative group">
            ...
            <input
              ...
              className="pl-10 pr-4 py-2 bg-surface/50 border border-border/50 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/50 transition-all w-64"
            />
          </div>
          
          <div className="flex items-center bg-surface/50 border border-border/50 rounded-xl p-1">
            <select 
              value={projectFilter}
              onChange={(e) => setProjectFilter(e.target.value)}
              className="bg-transparent text-xs font-bold uppercase tracking-wider px-3 py-1.5 focus:outline-none cursor-pointer hover:text-primary transition-colors"
            >
        ...
        <div className="flex flex-col gap-3">
          <AnimatePresence mode="popLayout">
            {filteredTasks.length > 0 ? (
                  ...
                  >
                    ...
                    >
                      <CardContent className="p-4 flex items-center gap-6">
                        <div className="flex items-center justify-center h-10 w-10 rounded-xl bg-surface-hover/50 text-text-muted group-hover:text-primary group-hover:bg-primary/10 transition-all">
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex items-center gap-2 min-w-[200px] justify-end">
                            ...
                            <ChevronRight className="h-4 w-4 text-text-disabled group-hover:text-primary transition-colors ml-2" />
                          </div>
                        </div>
                      </CardContent>
                    </Card>
              ...
              >
                <div className="h-20 w-20 rounded-full bg-surface-hover/50 flex items-center justify-center mb-4">
                  <Filter className="h-10 w-10 text-text-disabled" />
                </div>
                <h3 className="text-xl font-bold text-foreground">No tasks found</h3>
                <p className="text-text-muted max-w-xs mx-auto mt-2 font-medium">
                  We couldn&apos;t find any tasks matching your current search and filter criteria.
                </p>
                <button 
                  onClick={() => {
                    setSearchQuery("")
                  ...
                  className="mt-6 text-sm font-black uppercase tracking-widest text-primary hover:underline"
                >
                  Clear all filters
                </button>
              </motion.div>
