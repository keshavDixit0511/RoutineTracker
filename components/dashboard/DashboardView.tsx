...
export function DashboardView() {
  ...
  return (
    <div className="p-10 flex flex-col gap-10 w-full max-w-7xl mx-auto custom-scrollbar overflow-y-auto h-full">
      {/* Header Section */}
      <div className="space-y-2">
        ...
        <p className="text-text-muted font-medium">Real-time intelligence and project velocity metrics.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="bg-surface/30 border-border/50 backdrop-blur-md shadow-sm transition-all hover:shadow-lg hover:border-primary/30">
          ...
          <CardContent>
            <div className="text-4xl font-black text-foreground tracking-tight">{totalTasks}</div>
            <div className="mt-2 text-xs font-bold text-text-muted uppercase">Global Pool</div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-surface/30 border-border/50 backdrop-blur-md shadow-sm transition-all hover:shadow-lg hover:border-info/30">
          ...
          <CardContent>
            <div className="text-4xl font-black text-foreground tracking-tight">{inProgressTasks}</div>
            <div className="mt-2 text-xs font-bold text-info/80 uppercase">Active Cycles</div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-surface/30 border-border/50 backdrop-blur-md shadow-sm transition-all hover:shadow-lg hover:border-success/30">
          ...
          <CardContent>
            <div className="text-4xl font-black text-foreground tracking-tight">{completedTasks}</div>
            <div className="mt-2 text-xs font-bold text-success/80 uppercase">Finalized Units</div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-error/5 border-error/20 backdrop-blur-md shadow-sm transition-all hover:shadow-lg hover:border-error/40">
          ...
          <CardContent>
            <div className="text-4xl font-black text-error tracking-tight">{urgentTasks}</div>
            <div className="mt-2 text-xs font-bold text-error/80 uppercase">High Latency Risk</div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         ...
         <Card className="bg-surface/30 border-border/50 backdrop-blur-md col-span-1 rounded-2xl">
          <CardHeader className="border-b border-border/30 mb-4 pb-6">
            <CardTitle className="text-lg font-bold">Priority Allocation</CardTitle>
          </CardHeader>
               ...
               return (
                 <div key={priority} className="flex flex-col gap-3">
                   <div className="flex justify-between items-end">
                     <span className="text-xs font-bold uppercase tracking-wider text-text-muted/80">{priority}</span>
                     <span className="text-sm font-black text-foreground">{count}</span>
                   </div>
                   <div className="h-3 w-full bg-surface-hover/30 rounded-full p-[2px] border border-border/50">
                     <div 
         ...
         <Card className="bg-surface/30 border-border/50 backdrop-blur-md col-span-1 lg:col-span-2 rounded-2xl">
          <CardHeader className="border-b border-border/30 mb-4 pb-6">
            <CardTitle className="text-lg font-bold">In-Flight Operations</CardTitle>
          </CardHeader>
          <CardContent className="flex flex-col gap-4 pt-2">
             {tasks.filter(t => t.status !== "Done").slice(0, 5).map(task => (
               <div key={task.id} className="group flex items-center justify-between p-4 rounded-xl border border-border/50 bg-surface/20 hover:bg-surface/50 hover:border-primary/30 transition-all duration-300">
                 <div className="flex flex-col gap-1">
                   <span className="font-bold text-foreground text-[15px] group-hover:text-primary transition-colors">{task.title}</span>
                   <div className="flex items-center gap-2">
                     <span className="text-[10px] font-black uppercase tracking-widest text-text-muted/60">{task.id}</span>
                     <span className="h-1 w-1 rounded-full bg-text-disabled" />
                     <span className="text-[10px] font-bold text-text-muted uppercase tracking-tight">{task.status}</span>
                   </div>
                 </div>
                 <div className={`text-[10px] font-black px-3 py-1 rounded-lg uppercase tracking-tighter border ${
             ...
             {tasks.filter(t => t.status !== "Done").length === 0 && (
               <div className="text-center py-12 text-text-muted font-medium italic">Operational silence. All units clear.</div>
             )}
          </CardContent>
         </Card>
      </div>
